...
public class AllOperation {
	...
	public static void updateBooking() {
		...
		if(seatsNumber < bookedSeat) {
			for(int i = 0; i < (bookedSeat - seatsNumber); i++) {
				sList.remove(i);
			}
			seats = seatsNumber;
			booking.setSeats(sList);
			amount = (bookedSeat - seatsNumber) * flight.getBaseFare();
			System.out.println("New Booking Amount - " + amount);
			payment.setPaymentAmount(amount);
			System.out.println("Left Booking Amount will be Return - " + payment.getPaymentAmount());
		}else {
